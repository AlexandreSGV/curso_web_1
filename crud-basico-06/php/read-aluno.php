...
// Seleciona o aluno específico pelo ID, incluindo o nome de usuário associado
$stmt = $pdo->prepare("SELECT alunos.*, imagens.path, usuarios.username 
                       FROM alunos 
                       LEFT JOIN imagens ON alunos.imagem_id = imagens.id 
                       LEFT JOIN usuarios ON alunos.usuario_id = usuarios.id 
                       WHERE alunos.id = ?");
$stmt->execute([$id]);
