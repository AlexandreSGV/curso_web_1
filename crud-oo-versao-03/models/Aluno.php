...
class Aluno {
    ...
    // Método para obter alunos não matriculados em uma turma específica
    public function alunosNaoMatriculados($turma_id) {
        $query = "SELECT * FROM alunos WHERE id NOT IN (SELECT aluno_id FROM aluno_turma WHERE turma_id = :turma_id)";
        $statement = $this->conn->prepare($query);
